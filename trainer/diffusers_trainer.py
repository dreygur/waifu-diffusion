...
parser.add_argument(
    "--num_buckets", type=int, default=16, help="The number of buckets."
)
parser.add_argument(
    "--bucket_side_min",
    type=int,
...
parser.add_argument(
    "--output_path", type=str, default="./output", help="Root path for all outputs."
)
parser.add_argument(
    "--save_steps",
    type=int,
...
parser.add_argument(
    "--shuffle", dest="shuffle", type=bool_t, default="True", help="Shuffle dataset"
)
parser.add_argument(
    "--hf_token",
    type=str,
...
parser.add_argument(
    "--fp16", dest="fp16", type=bool_t, default="False", help="Train in mixed precision"
)
parser.add_argument(
    "--image_log_steps", type=int, default=500, help="Number of steps to log images at."
)
parser.add_argument(
    "--image_log_amount",
    type=int,
...
class Resize:
    def __init__(self, is_resizing: bool, is_not_migrating: bool) -> None:
        ...
        if not is_not_migrating:
            ...
            self.__directory = os.path.join(
                dataset_path[0], f"{dataset_path[1]}_cropped"
            )
    ...
    def __migration(self, image_path: str, w: int, h: int) -> Img:
        ...
        image.save(
            os.path.join(f"{self.__directory}", f"{filename}.jpg"), optimize=True
        )
        ...
        try:
            ...
            f = open(
                os.path.join(self.__directory, f"{filename}.txt"), "w", encoding="UTF-8"
            )
            f.close()
...
class ImageStore:
    def __init__(self, data_dir: str) -> None:
        ...
        self.image_files = []
        [
            self.image_files.extend(glob.glob(f"{data_dir}" + "/*." + e))
            for e in ["jpg", "jpeg", "png", "bmp", "webp"]
        ]

        self.validator = Validation(
            args.skip_validation, args.extended_validation
        ).validate
...
class AspectBucket:
    ...
    def init_buckets(self):
        ...
        unique_ratio_buckets = sorted(
            [
                sorted(buckets, key=_sort_by_area)[-1]
                for buckets in buckets_by_ratio.values()
            ],
            key=_sort_by_ratio,
        )
...
class AspectDataset(torch.utils.data.Dataset):
    ...
    def collate_fn(self, examples):
        ...
        if args.extended_mode_chunks < 2:
            ...
            max_chunks = args.extended_mode_chunks
            max_len = (
                np.ceil(max(len(x) for x in input_ids) / max_standard_tokens)
                .astype(int)
                .item()
                * max_standard_tokens
            )
            if max_len > max_standard_tokens:
                ...
                chunks = [
                    batch_t[:, i : i + max_standard_tokens]
                ...
                for chunk in chunks:
                    chunk = torch.cat(
                        (
                            torch.full(
                                (chunk.shape[0], 1), self.tokenizer.bos_token_id
                            ),
                            chunk,
                            torch.full(
                                (chunk.shape[0], 1), self.tokenizer.eos_token_id
                            ),
                        ),
                        1,
                    )
                ...
                if args.clip_penultimate:
                    input_ids = self.text_encoder.text_model.final_layer_norm(
                        self.text_encoder(
                            torch.asarray(input_ids).to(self.device),
                            output_hidden_states=True,
                        )["hidden_states"][-2]
                    )
                else:
...
def main():
    ...
    if args.hf_token is not None:
        ...
        try:
            ...
            print(
                "No HF Token detected in arguments or enviroment variable, setting it to none (as in string)"
            )
            args.hf_token = "none"
    ...
    text_encoder = text_encoder.to(
        device, dtype=weight_dtype if not args.train_text_encoder else torch.float32
    )

    unet = torch.nn.parallel.DistributedDataParallel(
        unet, device_ids=[rank], output_device=rank, gradient_as_bucket_view=True
    )
    ...
    train_dataloader = torch.utils.data.DataLoader(
        dataset, batch_sampler=sampler, num_workers=0, collate_fn=dataset.collate_fn
    )
    ...
    progress_bar = tqdm.tqdm(
        range(args.epochs * num_steps_per_epoch), desc="Total Steps", leave=False
    )
    global_step = 0
    ...
    lr_scheduler = get_scheduler(
        ...
        num_warmup_steps=int(
            args.lr_scheduler_warmup * num_steps_per_epoch * args.epochs
        ),
    ...
    def save_checkpoint(global_step):
        if rank == 0:
            ...
            pipeline = StableDiffusionPipeline(
                ...
                if type(text_encoder) is not torch.nn.parallel.DistributedDataParallel
                else text_encoder.module,
                vae=vae,
                unet=unet.module,
                tokenizer=tokenizer,
            ...
            print(
                f"saving checkpoint to: {args.output_path}/{args.run_name}_{global_step}"
            )
            pipeline.save_pretrained(
                f"{args.output_path}/{args.run_name}_{global_step}"
            )
                    ...
                    if global_step % args.image_log_steps == 0 and global_step > 0:
                        if rank == 0:
                            ...
                            batch_tokens = batch["tokens"][
                                random.randint(0, len(batch["tokens"]) - 1)
                            ]
                            if isinstance(batch_tokens[0], list):
                                batch_tokens = batch_tokens[0]
                            prompt = tokenizer.decode(batch_tokens)
                            ...
                            pipeline = StableDiffusionPipeline(
                                ...
                                is not torch.nn.parallel.DistributedDataParallel
                                else text_encoder.module,
                                vae=vae,
                                unet=unet.module,
                                tokenizer=tokenizer,
                                        ...
                                        if args.enablewandb:
                                            images.append(
                                                wandb.Image(
                                                    pipeline(
                                                        prompt,
                                                        num_inference_steps=args.image_log_inference_steps,
                                                    ).images[0],
                                                    caption=prompt,
                                                )
                                            )
                                            ...
                                            images.save(
                                                saveInferencePath + "/" + filenameImg
                                            )
                                            with open(
                                                saveInferencePath + "/" + filenameTxt,
                                                "a",
                                            ) as f:
